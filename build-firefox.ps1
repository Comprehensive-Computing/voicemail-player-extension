...
Copy-Item -LiteralPath (Join-Path $root "manifest.firefox.json") -Destination (Join-Path $target "manifest.json")
...
Write-Host "Firefox build ready at $target"
Write-Host "Firefox package ready at $archive"
